      ...
      <q-card-section class="q-pa-md">
        <div class="row q-col-gutter-sm items-center justify-between q-mb-sm">
          <div class="col-12 col-md-auto row q-gutter-sm">
            <q-btn unelevated color="primary" @click="$emit('add')" icon="add" label="Agregar" id="registrarCliente" />
            <q-btn outline color="green" @click="$emit('importFromExcel')" icon="upload" label="Importar Excel" id="importarExcel" />
            <q-btn outline color="info" @click="exportarClientesFiltrados" icon="file_download" label="Exportar Excel" id="exportarExcel" />
            <q-btn outline color="red" @click="exportarClientesPDF" icon="picture_as_pdf" label="Reporte PDF" id="exportarPdf" />
          </div>
          <div class="col-12 col-md-auto text-right">
            <q-input dense outlined bg-color="white" clearable debounce="300" v-model="search" id="buscar" placeholder="Buscar rápido..." style="min-width: 250px;">
        ...
        <div class="row q-col-gutter-sm">
          <div class="col-12 col-md-4">
            <q-select v-model="filtroTipoCliente" :options="tipoClienteFilterOptions" id="tipocliente" label="Filtrar por Tipo Cliente" dense outlined clearable>
              <template v-slot:prepend><q-icon name="category" /></template>
            </q-select>
          </div>
          <div class="col-12 col-md-4">
            <q-select v-model="filtroCanalVenta" :options="canalVentaFilterOptions" id="canalventa" label="Filtrar por Canal Venta" dense outlined clearable>
              <template v-slot:prepend><q-icon name="storefront" /></template>
            </q-select>
          </div>
          <div class="col-12 col-md-4">
            <q-select v-model="filtroTipoDocumento" :options="tipoDocumentoFilterOptions" id="tipodoc" label="Filtrar por Tipo Doc." dense outlined clearable>
    ...
    <q-dialog v-model="mostrarModal" full-width full-height>
      <q-card class="q-pa-md" style="height: 100%; max-width: 100%">
        ...
        <q-card-section class="q-pa-none" style="height: calc(100% - 60px)">
          <iframe v-if="pdfData" :src="pdfData" style="width: 100%; height: 100%; border: none"></iframe>
        </q-card-section>
      </q-card>
    </q-dialog>
  </div>
