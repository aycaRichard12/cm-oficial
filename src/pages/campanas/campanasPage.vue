<template>
  <q-page padding id="crearcampanas">
      ...
      <q-card-section class="row items-center q-pb-none">
        <div class="col">
          <div class="text-h5 text-weight-bold text-primary">
            <q-icon name="campaign" size="sm" class="q-mr-sm" />Gestión de Campañas
          </div>
    ...
    <CampanasTabla
      ...
      :editar-campana="cargarEditarCampana"
      :eliminar="eliminar"
      :tiene-categorias="tieneCategorias"
      :cargarcategoria="cargarcategoria"
      :cargar-precios="cargarPrecios"
    />

    <FormularioCampanaDialog
      v-model="formularioActivo"
      :form-data="formData"
      @update-form="(key, val) => (formData[key] = val)"
      :almacenes-options="almacenesOptions"
      :resetear-formulario="resetearFormulario"
      :registrar-campana="registrarCampana"
    />
    <CategoriasCampanaDialog
      v-model="dialogoCategorias"
      :categoria-form="categoriaForm"
      @update-form="(val) => (categoriaForm.idcategoriaprecio = val)"
      :categorias-precio-options="categoriasPrecioOptions"
      :categorias-campana="categoriasCampana"
      :registrar-categoria="registrarCategoria"
      :eliminar-categoria-campana="eliminarCategoriaCampana"
      :resetear-form="resetearCategoriaForm"
    />
    <PreciosCampanaDialog
      ...
      @update:producto-seleccionado="
        (val) => {
          productoSeleccionado = val
          alSeleccionarProducto(val)
        }
      "
...
const {
  ...
  categoriasCampana,
  categoriaForm,
  categoriasPrecioOptions,
  listarCategoriasPrecio,
  resetearCategoriaForm,
  cargarcategoria,
  registrarCategoria,
  eliminarCategoriaCampana,
  tieneCategorias,
  actualizarCampanasConCategorias,
} = useCategoriasCampana(q, campanas)

const {
  dialogoPrecios,
  preciosCampanaFiltrados,
  filtroPrecioCampania,
  productoSeleccionado,
  productosNoAsignadosOptions,
  precioForm,
  categoriasCampanaPrecioOptions,
  listarProductosAlmacen,
  cargarPrecios,
  onCategoriaCampañaSeleccionada,
  registrarPrecioCampaña,
  eliminarPrecioCampana,
  editarPrecioCampana,
