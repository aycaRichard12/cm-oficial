<template>
  <q-card flat bordered>
    <q-table :rows="campanas" :columns="columns" row-key="id" :filter="busqueda" v-model:pagination="pagination" flat :rows-per-page-options="[10, 20, 50]" :grid="$q.screen.lt.md">
      ...
      <template v-slot:body-cell-estado="props"><q-td :props="props"><q-chip id="estadoCampana" :color="Number(props.row.estado) === 1 ? 'positive' : 'negative'" text-color="white" :icon="Number(props.row.estado) === 1 ? 'check_circle' : 'cancel'" clickable @click="cambiarEstado(props.row.id, Number(props.row.estado) === 1 ? 2 : 1)">{{ Number(props.row.estado) === 1 ? 'Activa' : 'Inactiva' }}</q-chip></q-td></template>
      <template v-slot:body-cell-detalles="props"><q-td :props="props"><q-btn-group flat><q-btn id="btnCategoria" flat dense color="primary" icon="category" @click="cargarcategoria(props.row.id, props.row.idalmacen)"><q-tooltip>Categorías</q-tooltip></q-btn><q-btn id="btnProductos" flat dense color="primary" icon="shopping_cart" @click="cargarPrecios(props.row.id)" :disable="!tieneCategorias(props.row.id)"><q-tooltip>{{ tieneCategorias(props.row.id) ? 'Productos' : 'Falta categoría' }}</q-tooltip><q-badge v-if="tieneCategorias(props.row.id)" color="green" floating rounded /></q-btn></q-btn-group></q-td></template>
      <template v-slot:body-cell-acciones="props"><q-td :props="props"><q-btn id="btnEditar" flat dense round color="primary" icon="edit" @click="editarCampana(props.row)" /><q-btn id="btnEliminar" flat dense round color="negative" icon="delete" @click="eliminar(props.row.id)" /></q-td></template>
